...
export class DynamicList<T> extends DynamicValue<T> implements DynamicListReader<T> {
    ...
    push(v: ValueType<T>): void {
        ...
        ((this.value as unknown) as ValueType<T>[]).push(v);
        if (this._notifier) {
    ...
    map<U>(cb: MapCallback<T, U>): DynamicList<U[]> {
        ...
        const result = new DynamicList<U[]>(regenerate((this.value as unknown) as ValueType<T>[]));

        this.addListener((v) => {
            result.value = regenerate((v as unknown) as ValueType<T>[]);
        });
        return result;
    }
}
