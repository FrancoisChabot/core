...
import { CleanupOperation, popCleanupContext, pushCleanupContext } from './Cleanup';
...
class Prober<Intrinsics extends IntrinsicMap<Intrinsics>> {
    ...
    private _pendingCleanup: CleanupOperation[] = [];
    ...
    private _performWork<T extends Probed<Intrinsics>>(
        ...
        if (isPNode(cbResult)) {
            if (cbResult.data) {
                ...
                if (cbResult._cleanup) {
                    if (!resolved._cleanup) {
                        resolved._cleanup = cbResult._cleanup;
                    } else {
                        resolved._cleanup = resolved._cleanup.concat(cbResult._cleanup);
                        // Alternative: (probably not worth it unless massive cleanup counts are involved)
                        // where._cleanup.push(()=>{cbResult._cleanup?.forEach(c=>c())})
                    }
                }
            } else {
        ...
        if (this._pendingCleanup.length > 0) {
            if (!resolved._cleanup) {
                resolved._cleanup = this._pendingCleanup;
            } else {
                resolved._cleanup = resolved._cleanup.concat(this._pendingCleanup);
            }
